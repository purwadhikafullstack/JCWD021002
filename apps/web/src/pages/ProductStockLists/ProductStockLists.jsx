...
function ProductStockLists({size, handleWebSize}) {
  ...
  const handleAddToStock = async () => {
    try {
      ...
      const response = await axios.post(
        ...
        {
          productId: selectedProductStock.id,
  ...
  const fetchData = async () => {
    try {
          const response = await axios.get(
            `${import.meta.env.VITE_API_URL}/products/product-lists?page=${page}&pageSize=${pageSize}&sortField=${sortField}&sortOrder=${sortOrder}&categoryId=${categoryId}&productName=${productName}&storeId=${user?.store_idstore ? user?.store_idstore : storeId}&statusProduct=1&statusStock=${statusStock}`
          );
  ...
  console.log('status Stock',statusStock);
...
function formatPriceToIDR(price) {
    ...
    <Modal isOpen={isOpen} onClose={onClose}>
        ...
        <ModalContent>
          ...
          >
            ...
            <Select placeholder="Select option" value={statusStock} onChange={(e) => setStatusStock(e.target.value)}>
      ...
      <Stack spacing='4' direction='row' flexWrap='wrap' justifyContent={size == '500px' ? 'center' : 'flex-start'}>
            ...
            boxShadow='0px 1px 5px gray' border={item?.ProductStocks[0]?.status == 1 ? 'solid 2px green' : 'solid 2px red'} onClick={() => navigate(`/product-detail-admin/${item?.id}`)} _hover={{ cursor: 'pointer' }}>
              <Image
                  ...
                  <CardBody>
                    ...
                    <Heading size='sm' width='200px' isTruncated>{item.name}</Heading>
                      ...
                      <Flex flexWrap='wrap' column='row' justifyContent='center'>
                            <IconButton  icon={<IconEditCircle />} variant='ghost' colorScheme='blue' onClick={(event) => { setSelectedProductStock(item); setStockAmount(item?.ProductStocks[0]?.stock); setEditToStockModalIsOpen(true); event.stopPropagation(); }} />
                            <IconButton  icon={<IconTrashXFilled />} variant='ghost' colorScheme='red' onClick={(event) => { setSelectedProductStock(item); setDeleteModalOpen(true); event.stopPropagation(); }} />
                      </Flex>
                      <Flex justifyContent='center' flexDirection='row' flexWrap='wrap'>
                          <Text  color={item?.ProductStocks[0]?.status == 1 ? "green" : "red"}>{item?.ProductStocks[0]?.status == 1 ? (<IconCircleCheckFilled />) : (<IconCircleXFilled />)}</Text>
                          <Text color={item?.ProductStocks[0]?.status == 1 ? 'green' : 'red'} fontWeight='bold'>
                          {item?.ProductStocks[0]?.status == 1 ? 'Active' : 'Deactive'}
                          </Text>
  ...
  <Modal isOpen={editToStockModalIsOpen} onClose={() => setEditToStockModalIsOpen(false)}>
        {/* ... (other modal content) */}
        <ModalOverlay />
        <ModalContent>
        ...
        <ModalFooter>
          <Button colorScheme="blue" mr={3} onClick={handleEditToStock}>
            Edit to Stock
          </Button>
          <Button colorScheme="red" onClick={() => setEditToStockModalIsOpen(false)}>
            Cancel
          </Button>
        </ModalFooter>
        </ModalContent>
      ...
      <Modal isOpen={deleteModalOpen} onClose={() => setDeleteModalOpen(false)}>
        ...
        <ModalFooter>
          <Button colorScheme="blue" mr={3} onClick={handleDeleteProductStock}>
            Deactive from Stock
          </Button>
