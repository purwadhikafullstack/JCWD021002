      ...
      >
              ...
              <Flex columnGap="10px" mb="20px " flexDir={size == '500px' ? 'column' : 'row'} >
                <Box width="100%">
                  ...
                  <InputGroup>
                    <Input type={showPassword ? 'text' : 'password'} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" name="password" border="solid gray 1px" borderRadius="full" />
                    <InputRightElement h={'full'}>
                      <IconButton variant={'ghost'} onClick={() => setShowPassword((showPassword) => !showPassword) } borderRightRadius="full" icon={showPassword ? <IconEyeOff /> : <IconEye />} />
                    </InputRightElement>
                  </InputGroup>
                </Box>
                <Box pt="27px" width="100%" display={data?.role_idrole == 2 ? 'block' : 'none'} >
