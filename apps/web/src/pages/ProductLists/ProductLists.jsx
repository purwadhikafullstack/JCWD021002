  ...
  const handleDeleteProductStock = async () => {
    try {
      ...
      const response = await axios.patch(
        `${import.meta.env.VITE_API_URL}/products/product-soft-delete/${
          selectedProduct?.id
        }`,
        {
          headers: {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get(
        ...
        }/products/product-lists-v2?page=${page}&pageSize=${pageSize}&sortField=${sortField}&sortOrder=${sortOrder}&categoryId=${categoryId}&productName=${productName}&status=${status}`,
      );
      ...
      >
        ...
        <Box p={size == '500px' ? 0 : 5} pl={size == '500px' ? '0px' : '150px'}>
              ...
              data?.products.map((item, index) => (
                <>
                  <Card
                    ...
                    border={
                      item?.status == 1 ? 'solid 2px green' : 'solid 2px red'
                    }
                    onClick={() =>
                    ...
                    <CardBody>
                      <Stack mt="-3" spacing="0">
                        ...
                        <Flex
                          ...
                          flexWrap="wrap"
                        >
                          <Text color={item?.status == 1 ? 'green' : 'red'}>
                            {item?.status == 1 ? (
                              <IconCircleCheckFilled />
                            ) : (
                              <IconCircleXFilled />
          ...
          <Modal
            ...
            onClose={() => setAddToStockModalIsOpen(false)}
          >
            {/* ... (other modal content) */}
            <ModalOverlay />
            <ModalContent>
              ...
              <ModalFooter>
                ...
                <Button
                  colorScheme="red"
                  onClick={() => setAddToStockModalIsOpen(false)}
                >
                  Cancel
                </Button>
              </ModalFooter>
          ...
          >
            ...
            <ModalContent>
              ...
              <ModalFooter>
                <Button
                  isDisabled={selectedProduct?.status == true ? false : true}
                  colorScheme="blue"
                  mr={3}
                  onClick={handleDeleteProductStock}
                >
                  {selectedProduct?.status == true
                    ? 'Deactive Product'
                    : 'Product was deactive'}
                </Button>
                <Button
                  colorScheme="red"
