    ...
    <Flex w={'full'}>
      ...
      >
        ...
        <Flex gap={1.5}>
          ...
          <Text fontWeight={600}>
            {location?.latitude
              ? `${location?.City?.city}, ${location?.City?.Province?.province}`
              : '. . .'}
          </Text>
        </Flex>
...
function DrawerLoc({ isOpen, onClose, address, onOpen }) {
  ...
  const {isOpen : isModalOpen, onOpen : onMoldaOpen, onClose : onModalClose } = useDisclosure()
            ...
            <Flex w={'full'} direction={'column'} gap={5}>
              ...
              {/* <Link to={'/profile/detail/address/add'}> */}
                <Button
                  ...
                  onClick={
                    isLogin
                      ? () => navigate('/profile/detail/address/add?fromPage=/', {state: {onOpen: onOpen}})
                      : onMoldaOpen
                  }
                >
                  Tambah Alamat
